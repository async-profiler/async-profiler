    ...
    *this
        << (element("metadata")
            ...
            << (type("profiler.ProcessSample", T_PROCESS_SAMPLE, "Process Stats Sample")
                ...
                << field("normalizeCpuPercent", T_FLOAT, "Normalized CPU Percentage")
                << field("threads", T_INT, "Thread Count")
                << field("memSize", T_LONG, "Virtual Memory Size")
                << field("memResident", T_LONG, "Resident Memory")
                << field("memShared", T_LONG, "Shared Memory")
                << field("memText", T_LONG, "Text Memory")
                << field("memData", T_LONG, "Data Memory")
                << field("minorFaults", T_LONG, "Minor Page Faults")
                << field("majorFaults", T_LONG, "Major Page Faults")
                << field("ioRead", T_LONG, "I/O Read Bytes")
                << field("ioWrite", T_LONG, "I/O Write Bytes"))
