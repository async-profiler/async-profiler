...
static const char USAGE_STRING[] =
    ...
    "  --symbols-include glob    parse native symbols only for libraries matching glob (repeatable)\n"
    "  --symbols-exclude glob    skip parsing native symbols for libraries matching glob (repeatable)\n"
    "\n"
...
int main(int argc, const char** argv) {
    ...
    while (args.count() > 0 && !(jattach_action && pid)) {
        ...
        } else if (arg == "--symbols-include") {
            format << ",syminclude=" << args.next();

        } else if (arg == "--symbols-exclude") {
            format << ",symexclude=" << args.next();
