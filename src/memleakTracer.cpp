...
void MemLeakTracer::cleanup_entry(MemLeakTableEntry *entry) {
    JNIEnv* env = VM::jni();
...
void MemLeakTracer::initialize() {
    ...
    _max_stack_depth = min(MEMLEAK_STACKFRAMES_MAX_DEPTH, Profiler::instance()->max_stack_depth());
