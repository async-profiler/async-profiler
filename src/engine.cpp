...
int Engine::getNativeTrace(void* ucontext, int tid, const void** callchain, int max_depth, const void** first_java_pc) {
    const void* pc;
    uintptr_t fp;
    ...
    if (depth < max_depth && CodeHeap::contains(pc)) {
        *first_java_pc = pc;
        return depth;
    }

    callchain[depth] = NULL;
    return depth;
}