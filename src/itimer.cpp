...
void ITimer::signalHandlerJ9(int signo, siginfo_t* siginfo, void* ucontext) {
    ...
    const void* first_java_pc;
    notif.num_frames = _cstack == CSTACK_NO ? 0 : _cstack == CSTACK_DWARF
        ? StackWalker::walkDwarf(ucontext, notif.addr, MAX_J9_NATIVE_FRAMES, &first_java_pc)
        : StackWalker::walkFP(ucontext, notif.addr, MAX_J9_NATIVE_FRAMES, &first_java_pc);
