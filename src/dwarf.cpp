...
enum {
    DW_STACK_SLOT = sizeof(void*)
};


FrameDesc FrameDesc::_default = {0, DW_REG_FP | (2 * DW_STACK_SLOT) << 8, -2 * DW_STACK_SLOT};
...
#ifdef __x86_64__
    // DWARF stack unwinding is currently supported only on x64
    parse(eh_frame_hdr);
#endif
}
