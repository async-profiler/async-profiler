...
Example: `./build/bin/asprof -e wall -t -i 5ms -f result.html 8983`
...
To run the agent and pass commands to it, the helper script `asprof`
...
9234 Jps
8983 Computey
$ ./build/bin/asprof start 8983
$ ./build/bin/asprof stop 8983
```
...
If you need to profile some code as soon as the JVM starts up, instead of using the `asprof` script,
...
The `asprof` script actually converts command line arguments to that format.
...
directly by `asprof` script. E.g. `-d 5` results in 3 actions:
...
```
./build/bin/asprof -e cpu,alloc,lock -f profile.jfr ...
```
or use `--alloc` and `--lock` parameters with the desired threshold:
```
./build/bin/asprof -e cpu --alloc 2m --lock 10ms -f profile.jfr ...
```
...
9234 Jps
8983 Computey
$ ./build/bin/asprof -d 30 -f /tmp/flamegraph.html 8983
```
...
* `-d N` - the profiling duration, in seconds. If no `start`, `resume`, `stop`
  ...
  Example: `./build/bin/asprof -d 30 8983`
...
* `-i N` - sets the profiling interval in nanoseconds or in other units,
  ...
  Example: `./build/bin/asprof -i 500us 8983`
...
* `--chunksize N`, `--chunktime N` - approximate size and time limits for a single JFR chunk.
  ...
  Example: `./build/bin/asprof -f profile.jfr --chunksize 100m --chunktime 1h 8983`
...
* `-I include`, `-X exclude` - filter stack traces by the given pattern(s).
  ...
  Example: `./build/bin/asprof -I 'Primes.*' -I 'java/*' -X '*Unsafe.park*' 8983`

* `--title TITLE`, `--minwidth PERCENT`, `--reverse` - FlameGraph parameters.  
  Example: `./build/bin/asprof -f profile.html --title "Sample CPU profile" --minwidth 0.5 8983`
...
* `-f FILENAME` - the file name to dump the profile information to.  
  ...
  Example: `./build/bin/asprof -o collapsed -f /tmp/traces-%t.txt 8983`
...
* `--loop TIME` - run profiler in a loop (continuous profiling).
  ...
  Example: `./build/bin/asprof --loop 1h -f /var/log/profile-%t.jfr 8983`
...
* `--jfrsync CONFIG` - start Java Flight Recording with the given configuration
  ...
  Example: `./build/bin/asprof -e cpu --jfrsync profile -f combined.jfr 8983`
